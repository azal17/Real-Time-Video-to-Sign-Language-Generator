...
contraction_map = {
    "you're": "you are", "I'm": "I am","i'm": "i am", "he's": "he is", "she's": "she is", 
...
# Remove symbols except for contractions

def clean_text(text):
    
    text = re.sub(r"[^\w\s']", '', text)  # Keeps words, spaces, and apostrophes (for contractions)
    return text
...
def preprocess_text(text):
    text = expand_contractions(text)  # Expand contractions first
    text = clean_text(text)  # Remove symbols
    return text
...
def animation(request):
    if request.method == 'POST':
        ...
        cleaned_text = preprocess_text(text).lower()  # Preprocess input

        video_path = cleaned_text + ".mp4"
        video_file = finders.find(video_path)

        if video_file:
            return render(request, 'animation.html', {'words': [cleaned_text], 'text': text})
